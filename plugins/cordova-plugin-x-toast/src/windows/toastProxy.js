...
var toastProxy = {
    ...
    show: function(successCallback, errorCallback, options) {
        ...
        toastTextElements[0].appendChild(
            toastXml.createTextNode(options[0].message),
        );
        ...
        notifications.ToastNotificationManager.createToastNotifier().show(
            toast,
        );
    },
    ...
    hide: function() {
        if (this.lastDisplayedNotification !== null) {
            notifications.ToastNotificationManager.createToastNotifier().hide(
                toast,
            );
