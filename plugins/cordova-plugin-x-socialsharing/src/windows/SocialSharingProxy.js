...
module.exports = {
    share: function(win, fail, args) {
        ...
        var getExtension = function(strBase64) {
            return strBase64.substring(
                strBase64.indexOf('/') + 1,
                strBase64.indexOf(';base64'),
            );
        };
        ...
        var getFileName = function(position, fileExtension) {
            var fileName =
                (subject ? sanitizeFilename(subject) : 'file') +
        ...
        var createTemporalFile = function(fileName, buffer) {
            ...
            return folder
                ...
                .then(function(file) {
                    filePath = file.path;
                    return Windows.Storage.FileIO.writeBufferAsync(
                        file,
                        buffer,
                    );
                })
            ...
            if (fileOrFileArray.length > 0) {
                ...
                for (var i = 0; i < fileOrFileArray.length; i++) {
                    ...
                    if (file.indexOf('data:') >= 0) {
                        ...
                        var buffer = Windows.Security.Cryptography.CryptographicBuffer.decodeFromBase64String(
                            file.split(',')[1],
                        );
                        if (buffer) {
    ...
    shareViaEmail: function(win, fail, args) {
        ...
        var doShare = function(e) {
            e.request.data.properties.title = subject ? subject : 'Sharing';
            if (message) {
                var htmlFormat = Windows.ApplicationModel.DataTransfer.HtmlFormatHelper.createHtmlFormat(
                    message,
                );
                ...
                for (var i = 0; i < fileOrFileArray.length; i++) {
                    Windows.Storage.StorageFile.getFileFromPathAsync(
                        ...
                        function(index, file) {
                            ...
                            var streamRef = Windows.Storage.Streams.RandomAccessStreamReference.createFromFile(
                                file,
                            );
