(function() {
    ...
    newSQLError = function(error, code) {
        ...
        if (!sqlError) {
            sqlError = new Error(
                'a plugin had an error but provided no response',
            );
            sqlError.code = code;
        }
   ...
   */
            ...
            opensuccesscb = (function(_this) {
                return function() {
                    ...
                    if (!_this.openDBs[_this.dbname]) {
                        console.log(
                            'database was closed during open operation',
                        );
                    }
    ...
    SQLitePluginTransaction.prototype.handleStatementFailure = function(
        ...
        if (handler(this, response) !== false) {
            throw newSQLError(
                'a statement error callback did not return false: ' +
                    response.message,
                response.code,
            );
        }
        ...
        handlerFor = function(index, didSucceed) {
            return function(response) {
                ...
                if (!txFailure) {
                    try {
                        if (didSucceed) {
                            tx.handleStatementSuccess(
                                batchExecutes[index].success,
                                response,
                            );
                        } else {
    ...
    SQLitePluginTransaction.prototype.finish = function() {
        ...
        failed = function(tx, err) {
            ...
            if (tx.error && typeof tx.error === 'function') {
                tx.error(
                    newSQLError(
                        'error while trying to commit: ' + err.message,
                        err.code,
                    ),
                );
            }
     ...
     */
        openDatabase: argsArray(function(args) {
            ...
            if (args.length < 1 || !args[0]) {
                throw newSQLError(
                    'Sorry missing mandatory open arguments object in openDatabase call',
                );
            }
            if (args[0].constructor === String) {
                throw newSQLError(
                    'Sorry first openDatabase argument must be an object',
                );
            }
            openargs = args[0];
            if (!openargs.name) {
                throw newSQLError(
                    'Database name value is missing in openDatabase call',
                );
            }
            if (
            ...
            if (!dblocation) {
                throw newSQLError(
                    'Valid iOS database location could not be determined in openDatabase call',
                );
            }
            ...
            if (
                !!openargs.androidDatabaseProvider &&
            ...
            if (
                ...
                throw newSQLError(
                    "Incorrect androidDatabaseProvider value. Valid values are: 'default', 'system'",
                );
            }
            if (
                !!openargs.androidDatabaseProvider &&
        ...
        deleteDatabase: function(first, success, error) {
            ...
            if (first.constructor === String) {
                throw newSQLError(
                    'Sorry first deleteDatabase argument must be an object',
                );
            } else {
                if (!(first && first['name'])) {
            ...
            if (!dblocation) {
                throw newSQLError(
                    'Valid iOS database location could not be determined in deleteDatabase call',
                );
            }
                                ...
                                function(ignored, resutSet) {
                                    if (!resutSet.rows) {
                                        return SelfTest.finishWithError(
                                            errorcb,
                                            'Missing resutSet.rows',
                                        );
                                    }
                                    if (!resutSet.rows.length) {
                                        return SelfTest.finishWithError(
                                            errorcb,
                                            'Missing resutSet.rows.length',
                                        );
                                    }
                ...
                function(db) {
                    db.transaction(
                        ...
                        function(txError) {
                            return SelfTest.finishWithError(
                                errorcb,
                                'UNEXPECTED TRANSACTION ERROR: ' + txError,
                            );
                        },
                    );
                },
        ...
        cleanupAndFinish: function(successcb, errorcb) {
            SQLiteFactory.deleteDatabase(
                ...
                function(cleanup_err) {
                    SelfTest.finishWithError(
                        errorcb,
                        'CLEANUP DELETE ERROR: ' + cleanup_err,
                    );
                },
            );
        },
        ...
        echoTest: function(okcb, errorcb) {
            ...
            ok = function(s) {
                if (s === 'test-string') {
                    ...
                    return errorcb(
                        "Mismatch: got: '" + s + "' expected 'test-string'",
                    );
                }
            };
            error = function(e) {
