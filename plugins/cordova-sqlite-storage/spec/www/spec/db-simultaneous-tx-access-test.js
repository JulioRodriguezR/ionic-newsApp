...
var mytests = function() {
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(
            scenarioList[i] + ': simultaneous tx access test(s)',
            function() {
                ...
                it(suiteName + ' open same db twice with string test', function(
                    done,
                ) {
                        ...
                        function(tx) {
                            ...
                            tx.executeSql(
                                ...
                                function(tx, result) {
                                    ...
                                    expect(result.rows.item(0).uppertext).toBe(
                                        'FIRST',
                                    ); // check db1 read tx result
                                    if (++checkCount === expectedCheckCount)
                                        done();
                                },
                                function(error) {
                ...
                it(
                    suiteName +
                        ' test simultaneous transactions (same db handle)',
                    function(done) {
                        var db = openDatabase(
                            'Database-Simultaneous-Tx',
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                ...
                it(
                    suiteName +
                        ' test simultaneous transactions, different db handles (same db)',
                    function(done) {
                ...
                it(
                    suiteName + ' can open two databases at the same time',
                    function(done) {
                        ...
                        db1.transaction(function(tx1) {
                            tx1.executeSql(
                                'CREATE TABLE IF NOT EXISTS test1 (x int)',
                            );
                        });
                        ...
                        db2.transaction(function(tx2) {
                            tx2.executeSql(
                                'CREATE TABLE IF NOT EXISTS test2 (x int)',
                            );
                        });
                                ...
                                db2.transaction(
                                    function(tx2) {
                                        ...
                                        tx2.executeSql(
                                            ...
                                            function(tx, result) {
                                                expect(
                                                    result.rows.item(0).x,
                                                ).toBe(2); // selected from second database
                                                ++checkCount;
                                            },
                                        );
                                    },
                ...
                it(
                    suiteName +
                        ' same database file with separate writer/reader db handles',
                    function(done) {
                        var dbname = 'writer-reader-test.db';
                        var dbw = openDatabase(
                            dbname,
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                        var dbr = openDatabase(
                            dbname,
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        ...
                        dbw.transaction(
                            ...
                            function() {
                                dbr.readTransaction(
                                    function(tx) {
                                        tx.executeSql(
                                            'SELECT test_data from tt',
                                            [],
                                            function(tx, result) {
                                                expect(
                                                    result.rows.item(0)
                                                        .test_data,
                                                ).toBe('My-test-data'); // read data from reader handle
                                                done();
                                            },
                                        );
                                    },
                ...
                it(
                    suiteName +
                        ' same database file with multiple writer db handles',
                    function(done) {
                        var dbname = 'multi-writer-test.db';
                        var dbw1 = openDatabase(
                            dbname,
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                        var dbw2 = openDatabase(
                            dbname,
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
