    ...
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(scenarioList[i] + ': ext tx blob test(s)', function() {
            ...
            describe(scenarioList[i] + ': Blob object test(s)', function() {
                ...
                it(
                    suiteName +
                        'INSERT Blob from ArrayBuffer (non-standard parameter type)',
                    function(done) {
                        if (/Android 4.[1-3]/.test(navigator.userAgent))
                            pending('SKIP for Android 4.1-4.3');

                        // IMPORTANT:
                        if (typeof Blob === 'undefined')
                            pending('Blob type does not exist');
                        ...
                        if (typeof ArrayBuffer === 'undefined')
                            pending('ArrayBuffer type does not exist');

                        var db = openDatabase(
                            'Blob-object-from-ArrayBuffer-test.db',
                        );
                        ...
                        db.transaction(
                            function(tx) {
                                ...
                                tx.executeSql(
                                    'CREATE TABLE IF NOT EXISTS test_table (foo blob)',
                                );
                                tx.executeSql(
