    ...
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(scenarioList[i] + ': db tx sql features test(s)', function() {
            ...
            it(
                ...
                function(done) {
                    if (isWebSql && !isBrowser)
                        pending('SKIP for Android/iOS (WebKit) Web SQL'); // XXX TBD NOT WORKING on all Android/iOS versions
                    if (isAndroid && isImpl2)
                        pending('SKIP for android.database implementation'); // NOT WORKING on all versions

                    var db = openDatabase(
                        'tx-with-a-with-clause-test.db',
                        '1.0',
                        'Test',
                        DEFAULT_SIZE,
                    );
                    ...
                    db.readTransaction(
                        function(tx) {
                            tx.executeSql(
                                'WITH one(x) AS (SELECT 1) SELECT x FROM one;',
                            );
                        },
                        function(error) {
                            // NOT EXPECTED:
            ...
            it(
                ...
                function(done) {
                    if (isWebSql && isSafariBrowser)
                        pending('SKIP for (WebKit) Web SQL on Safari browser');
                    if (isWebSql && isAndroid)
                        pending('SKIP for Android Web SQL');
                    if (
                        isWebSql &&
                        isAppleMobileOS &&
                        /OS 1[1-9]/.test(navigator.userAgent)
                    )
                        pending('SKIP (WebKit) Web SQL on iOS 11(+)');

                    var db = openDatabase(
                        'virtual-table-using-fts3.db',
                        '1.0',
                        'Test',
                        DEFAULT_SIZE,
                    );
                ...
                function(done) {
                    ...
                    db.transaction(function(tx) {
                        ...
                        tx.executeSql(
                            ...
                            [' { "this" : "is", "a": [ "test" ] } '],
                            function(tx, res) {
                                expect(res.rows.item(0).my_json).toEqual(
                                    '{"this":"is","a":["test"]}',
                                );
                                //done();
                                // Close (plugin only) & finish:
                ...
                function(done) {
                    ...
                    db.transaction(function(tx) {
                        ...
                        tx.executeSql(
                            ...
                            function(tx, res) {
                                // EXPECTED RESULT:
                                expect(res.rows.item(0).my_object).toEqual(
                                    '{"ex":"[52,3.14159]"}',
                                );
                                //done();
                                // Close (plugin only) & finish:
            ...
            it(
                ...
                function(done) {
                    ...
                    if (isAndroid && isImpl2)
                        pending(
                            'NOT IMPLEMENTED for all versions of android.database',
                        ); // NOT IMPLEMENTED for all versions of Android database (failed in Circle CI)

                    var db = openDatabase(
                        'virtual-table-using-r-tree.db',
                        '1.0',
                        'Test',
                        DEFAULT_SIZE,
                    );
