...
var mytests = function() {
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(
            scenarioList[i] + ': db tx multiple update test(s)',
            function() {
                ...
                it(
                    suiteName +
                        'Multiple updates with key (evidently needs temporary transaction files to work)',
                    function(done) {
                        var db = openDatabase(
                            'MultipleUpdatesWithKey',
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                        ...
                        db.transaction(
                            function(tx) {
                                ...
                                tx.executeSql(
                                    'CREATE TABLE IF NOT EXISTS Task (id primary key, subject)',
                                );
                                tx.executeSql('INSERT INTO Task VALUES (?,?)', [
                                    '928238b3-a227-418f-aa15-12bb1943c1f2',
                                    'test1',
                                ]);
                                tx.executeSql('INSERT INTO Task VALUES (?,?)', [
                                    '511e3fb7-5aed-4c1a-b1b7-96bf9c5012e2',
                                    'test2',
                                ]);
