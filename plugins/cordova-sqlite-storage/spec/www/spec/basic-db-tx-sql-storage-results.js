...
var mytests = function() {
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(
            scenarioList[i] + ': BASIC db tx sql storage results test(s)',
            function() {
                ...
                it(
                    suiteName +
                        'Simple INSERT results test: check insertId & rowsAffected in result',
                    function(done) {
                        var db = openDatabase(
                            'INSERT-results-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                ...
                it(
                    ...
                    function(done) {
                        var db = openDatabase(
                            'basic-db-sql-tx-results-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                                    ...
                                    function(tx, res) {
                                        ...
                                        db.transaction(
                                            function(tx) {
                                                ...
                                                tx.executeSql(
                                                    'SELECT data_num FROM test_table;',
                                                    [],
                                                    function(tx, res) {
                                                        ++check_count;
                                                ...
                                                tx.executeSql(
                                                    "DELETE FROM test_table WHERE data LIKE 'tes%'",
                                                    [],
                                                    function(tx, res) {
                                                        ++check_count;
                ...
                it(
                    ...
                    function(done) {
                        if (isWebSql && /Android 5.1/.test(navigator.userAgent))
                            pending('SKIP on (WebKit) Web SQL on Android 5.1'); // XXX TBD INCONSISTENT RESULT on (WebKit) Web SQL on Android 5.1(.1) x86 emulator vs Samsung test device

                        var db = openDatabase(
                            'db-tx-result-lifetime-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                                    ...
                                    function(tx, rs) {
                                        ...
                                        db.transaction(
                                            function(tx) {
                                                ...
                                                tx.executeSql(
                                                    ...
                                                    function(tx, res) {
                                                        ...
                                                        store_row_item = res.rows.item(
                                                            0,
                                                        );
                                                        expect(
                                                            store_row_item.data_num,
                                                        ).toBe(101);
                                                        expect(
                                                            store_row_item.data,
                                                        ).toBe('test');
                                                ...
                                                tx.executeSql(
                                                    ...
                                                    function(tx, res) {
                                                        ...
                                                        var temp1 = res.rows.item(
                                                            0,
                                                        );
                                                        var temp2 = res.rows.item(
                                                            0,
                                                        );

                                                        expect(temp1.data).toBe(
                                                            'test',
                                                        );
                                                        expect(temp2.data).toBe(
                                                            'test',
                                                        );
                                            ...
                                            function() {
                                                ...
                                                expect(
                                                    store_rows.item(0).data,
                                                ).toBe('test');

                                                expect(store_data_text).toBe(
                                                    'test',
                                                );
                                                expect(
                                                    store_row_item.data,
                ...
                it(
                    ...
                    function(done) {
                        var db = openDatabase(
                            'tx-sql-starting-with-extra-space-results-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                ...
                it(
                    suiteName +
                        'tx sql starting with extra semicolon results test',
                    function(done) {
                        var db = openDatabase(
                            'tx-sql-starting-with-extra-semicolon-results-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                ...
                describe(
                    scenarioList[i] +
                        ': NON-standard Multi-row INSERT with parameters (post-sqlite 3.6 feature)]',
                    function() {
                        it(
                            suiteName +
                            ...
                            function(done) {
                                if (isWebSql && isAndroid)
                                    pending('SKIP for Android Web SQL'); // FUTURE TBD (??)

                                var db = openDatabase(
                                    'Multi-row-INSERT-with-parameters-test.db',
                                    '1.0',
                                    'Test',
                                    DEFAULT_SIZE,
                                );
                                ...
                                db.transaction(
                                    function(tx) {
                                        ...
                                        tx.executeSql(
                                            'CREATE TABLE TestTable (x,y);',
                                        );

                                        tx.executeSql(
                                            'INSERT INTO TestTable VALUES (?,?),(?,?)',
                                            ['a', 1, 'b', 2],
                                            function(ignored1, rs1) {
                                                ...
                                                if (
                                                    ...
                                                    expect(
                                                        rs1.rowsAffected,
                                                    ).toBe(1); // [ACTUAL (BROKEN) for built-in (AOSP) android.database]
                ...
                describe(
                    suiteName + 'NON-STANDARD SQL statement list test(s)',
                    function() {
                        it(
                            suiteName +
                                ...
                                (isWebSql
                                    ? 'Web SQL ERROR'
                                    : 'DEVIATION - PLUGIN BROKEN (with potential data loss)'),
                            function(done) {
                                var db = openDatabase(
                                    'INSERT-statement-list-test.db',
                                    '1.0',
                                    'Test',
                                    DEFAULT_SIZE,
                                );
                        ...
                        it(
                            suiteName +
                                'executeSql with SELECT statement list - NOT ALLOWED [PLUGIN BROKEN]',
                            function(done) {
                                ...
                                var db = openDatabase(
                                    'tx-sql-with-select-statement-list.db',
                                );
                                ...
                                db.transaction(function(tx) {
                                    tx.executeSql(
                                        ...
                                        function(ignored, rs) {
                                            ...
                                            if (isWebSql)
                                                expect(
                                                    'WebKit Web SQL implementation changed (DEVIATION)',
                                                ).toBe('--');
                                        ...
                                        function(ignored, error) {
                                            if (!isWebSql)
                                                expect(
                                                    'PLUGIN FIXED, please update this test',
                                                ).toBe('--');
                ...
                describe(
                    suiteName + 'Binary BLOB data INSERT test(s)',
                    function() {
                        it(
                            suiteName +
                                "INSERT Binary literal BLOB data (X'010203'), check results, and check stored data HEX value",
                            function(done) {
                                var db = openDatabase(
                                    'Binary-literal-BLOB-data-INSERT-test.db',
                                );
                                ...
                                db.transaction(
                                    function(tx) {
                                        ...
                                        tx.executeSql(
                                            'CREATE TABLE TestTable (x);',
                                        );

                                        tx.executeSql(
                                            "INSERT INTO TestTable VALUES (X'010203')",
                                            [],
                                            function(ignored, rs1) {
                                                // EXPECTED: CORRECT RESULT:
                                                expect(rs1).toBeDefined();
                                                expect(rs1.insertId).toBe(1);
                                                expect(rs1.rowsAffected).toBe(
                                                    1,
                                                );

                                                tx.executeSql(
                                                    'SELECT HEX(x) AS hex_value FROM TestTable',
                                                    [],
                                                    function(
                                                        ignored,
                                                        resultSet,
                ...
                describe(
                    suiteName + 'STANDARD multi-row INSERT tests',
                    function() {
                        it(
                            suiteName +
                            ...
                            function(done) {
                                ...
                                db.transaction(
                                    function(tx) {
                                        ...
                                        tx.executeSql(
                                            'CREATE TABLE tt1 (data);',
                                        );
                                        tx.executeSql(
                                            'CREATE TABLE tt2 (data);',
                                        );
                                        ...
                                        tx.executeSql(
                                            'INSERT INTO tt2 SELECT data FROM tt1;',
                                            [],
                                            function(ignored1, rs1) {
                                                // EXPECTED (CORRECT RESULT):
                                                expect(rs1).toBeDefined();
                        ...
                        it(
                            suiteName +
                                'INSERT with TRIGGER & check results [rowsAffected INCORRECT with Android 4.1-4.3 (WebKit) Web SQL & androidDatabaseImplementation: 2 (built-in android.database) setting]',
                            function(done) {
                                var db = openDatabase(
                                    'INSERT-with-TRIGGER-test.db',
                                    '1.0',
                                    'Test',
                                    DEFAULT_SIZE,
                                );
                                    ...
                                    function(tx) {
                                        ...
                                        tx.executeSql(
                                            ...
                                            function(ignored1, rs1) {
                                                ...
                                                tx.executeSql(
                                                    'SELECT COUNT(*) AS count1 FROM tt1',
                                                    [],
                                                    function(ignored, rs2) {
                                                        // EXPECTED: CORRECT RESULT:
                                                        expect(
                                                        ...
                                                        tx.executeSql(
                                                            'SELECT COUNT(*) AS count2 FROM tt2',
                                                            [],
                                                            function(
                                                                ignored,
                                                                rs3,
                ...
                it(
                    suiteName +
                        'INSERT OR IGNORE result in case of constraint violation [(WebKit) Web SQL DEVIATION on Android/iOS: reports old insertId value]',
                    function(done) {
                        var db = openDatabase(
                            'INSERT-OR-IGNORE-test.db',
                            '1.0',
                            'Test',
                            DEFAULT_SIZE,
                        );
                        ...
                        db.transaction(
                            function(tx) {
                                ...
                                tx.executeSql(
                                    'CREATE TABLE tt (data1 NUMERIC UNIQUE, data2 TEXT);',
                                );

                                tx.executeSql(
                                    'INSERT OR IGNORE INTO tt VALUES (?,?)',
                                    [101, 'Alice'],
                                    function(ignored, rs) {
                                ...
                                tx.executeSql(
                                    ...
                                    function(ignored, rs1) {
                                        ...
                                        if (isWebSql)
                                            expect(checkInsertId).toBe(2);
                                        // Andriod/iOS WebKit Web SQL DEVIATION: OLD insertId value
                                        else
                                            expect(checkInsertId).toBe(
                                                undefined,
                                            );
                                        ...
                                        tx.executeSql(
                                            'SELECT COUNT(*) AS MyCount FROM tt',
                                            [],
                                            function(ignored, rs2) {
                                                expect(rs2).toBeDefined();
                        ...
                        function(done) {
                            ...
                            createdb.transaction(
                                ...
                                function(e) {
                                    ...
                                    isWebSql
                                        ? done()
                                        : createdb.close(done, done);
                                },
                                function() {
                                    if (isWebSql) {
                                        addColumnTest();
                                    } else {
                                        createdb.close(addColumnTest, function(
                                            e,
                                        ) {
                                            // ERROR RESULT (NOT EXPECTED):
                                            expect(false).toBe(true);
                            ...
                            function addColumnTest() {
                                ...
                                db.transaction(
                                    function(tx) {
                                        tx.executeSql(
                                            'ALTER TABLE TestTable ADD COLUMN data2;',
                                        );
                                        tx.executeSql(
                                            'UPDATE TestTable SET data2=?;',
                                            ['test-value-2'],
                                        );
                        ...
                        function(done) {
                            ...
                            function tableRenameTest() {
                                ...
                                db.transaction(
                                    function(tx) {
                                        tx.executeSql(
                                            'ALTER TABLE TestTable RENAME TO tt2;',
                                        );
