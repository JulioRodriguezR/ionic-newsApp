    ...
    for (var i = 0; i < scenarioCount; ++i) {
        ...
        describe(scenarioList[i] + ': sqlite version test(s)', function() {
            ...
            describe(suiteName + 'basic sqlite version test(s)', function() {
                it(
                    suiteName +
                        'Check sqlite version correctly matches pattern',
                    function(done) {
                        var db = openDatabase(
                            'check-sqlite-version-matches-pattern.db',
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                        ...
                        db.transaction(
                            function(tx) {
                                ...
                                tx.executeSql(
                                    'SELECT SQLITE_VERSION() AS myResult',
                                    [],
                ...
                it(
                    suiteName + 'Check actual sqlite version',
                    function(done) {
                        if (isWebSql)
                            pending('NOT DETERMINISTIC for (WebKit) Web SQL');
                        if (!isWebSql && isAndroid && isImpl2)
                            pending(
                                'NOT DETERMINISTIC for plugin on Android with androidDatabaseImplementation: 2',
                            );

                        var db = openDatabase(
                            'check-actual-sqlite-version.db',
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
            ...
            describe(suiteName + 'sqlite encoding test(s)', function() {
                it(
                    suiteName +
                        'Check internal database encoding: UTF-16le for Windows, UTF-8 for others (plugin ONLY)',
                    function(done) {
                        if (isWebSql)
                            pending('SKIP: NOT SUPPORTED for (WebKit) Web SQL');

                        var db = openDatabase(
                            'Check-sqlite-PRAGMA-encoding.db',
                            '1.0',
                            'Demo',
                            DEFAULT_SIZE,
                        );
                        ...
                        db.executeSql(
                            ...
                            function(rs) {
                                ...
                                expect(rs.rows.length).toBe(1);
                                if (isWindows)
                                    expect(rs.rows.item(0).encoding).toBe(
                                        'UTF-16le',
                                    );
                                else
                                    expect(rs.rows.item(0).encoding).toBe(
            ...
            describe(
                suiteName + 'default page/cache size check(s)',
                function() {
                    it(
                        suiteName + 'Check default page size (plugin ONLY)',
                        function(done) {
                            // ref: litehelpers/Cordova-sqlite-storage#781
                            if (isWebSql)
                                pending(
                                    'SKIP: NOT SUPPORTED for (WebKit) Web SQL',
                                );
                            ...
                            db.executeSql(
                                ...
                                function(rs) {
                                    ...
                                    expect(rs.rows).toBeDefined();
                                    expect(rs.rows.length).toBe(1);
                                    expect(rs.rows.item(0).page_size).toBe(
                                        4096,
                                    );
                    ...
                    it(
                        suiteName + 'Check default cache size (plugin ONLY)',
                        function(done) {
                            // ref: litehelpers/Cordova-sqlite-storage#781
                            if (isWebSql)
                                pending(
                            ...
                            db.executeSql(
                                ...
                                function(rs) {
                                    ...
                                    // TBD OLD VALUE on Android (...)
                                    else
                                        expect(resultRow.cache_size).toBe(
                                            -2000,
                                        ); // NEW VALUE, otherwise
            ...
            describe(suiteName + 'additional sqlite check(s)', function() {
                it(
                    suiteName +
                        'Check default PRAGMA journal_mode setting (plugin ONLY)',
                    function(done) {
                        if (isWebSql)
                            pending('SKIP: NOT SUPPORTED for (WebKit) Web SQL');
                            ...
                            function(rs) {
                                ...
                                if (!isWindows && isAndroid && isImpl2)
                                    expect(rs.rows.item(0).journal_mode).toBe(
                                        /Android 9/.test(navigator.userAgent)
                                            ? 'wal'
                                            : /Android 8.1.99/.test(
                                                  navigator.userAgent,
                                              )
