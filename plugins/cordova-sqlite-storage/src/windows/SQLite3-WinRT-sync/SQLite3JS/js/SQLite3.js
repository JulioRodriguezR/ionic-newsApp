    ...
    Statement = WinJS.Class.define(
        ...
        },
        {
            bind: function(args) {
                ...
                args.forEach(function(arg, i) {
                    ...
                    switch (type(arg)) {
                        case 'number':
                            if (arg % 1 === 0) {
                                resultCode = this.statement.bindInt64(
                                    index,
                                    arg,
                                );
                            } else {
                                resultCode = this.statement.bindDouble(
                                    index,
                                    arg,
                                );
                            }
                        ...
                        default:
                            throw new Error(
                                'Unsupported argument type: ' + type(arg),
                            );
                    }
                    if (resultCode !== SQLite3.ResultCode.ok) {
                        throw new Error(
                            'Error ' +
                                resultCode +
                                ' when binding argument to SQL query.',
                        );
                    }
                }, this);
            },
            ...
            each: function(callback) {
                ...
                if (
                    resultCode !== SQLite3.ResultCode.done &&
                    resultCode !== SQLite3.ResultCode.ok
                ) {
                    throw new Error(
                        'SQLite3 step error result code: ' + resultCode,
                    );
                }
            },
            ...
            _getRow: function() {
                ...
                for (
                    i = 0, len = this.statement.columnCount();
                    i < len;
                    i += 1
                ) {
            ...
            _getColumn: function(index) {
                switch (this.statement.columnType(index)) {
                    ...
                    default:
                        throw new Error(
                            'Unsupported column type in column ' + index,
                        );
                }
            },
        },
    ...
    Database = WinJS.Class.define(
        function(dbPath) {
            try {
                ...
                throwSQLiteError(
                    'Error creating an SQLite database connection.',
                    comException,
                );
            }
        },
