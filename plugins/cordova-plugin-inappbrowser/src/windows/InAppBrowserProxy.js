...
function attachNavigationEvents(element, callback) {
    if (isWebViewAvailable) {
                ...
                callback(
                    {
                        ...
                        message:
                            'Navigation failed with error code ' +
                            e.webErrorStatus,
                    },
                    { keepCallback: true },
                );
        ...
        var onError = function() {
            callback(
                { type: 'loaderror', url: this.contentWindow.location },
                { keepCallback: true },
            );
        };
        ...
        element.addEventListener('unload', function() {
            callback(
                { type: 'loadstart', url: this.contentWindow.location },
                { keepCallback: true },
            );
        });
        ...
        element.addEventListener('load', function() {
            callback(
                { type: 'loadstop', url: this.contentWindow.location },
                { keepCallback: true },
            );
        });
        ...
        setImmediate(function() {
            ...
            if (target === '_system') {
                ...
                if (!browserWrap) {
                    ...
                    browserWrapStyle.href = urlutil.makeAbsolute(
                        '/www/css/inappbrowser.css',
                    );
                ...
                popup = document.createElement(
                    isWebViewAvailable ? 'x-ms-webview' : 'iframe',
                );
                ...
                if (
                    features.indexOf('hardwareback=yes') > -1 ||
        ...
        setImmediate(function() {
            ...
            if (isWebViewAvailable && browserWrap && popup) {
                ...
                var uri = new Windows.Foundation.Uri(
                    filePath.replace('ms-appx-web:', 'ms-appx:'),
                );
