    ...
    var createTests = function(platformOpts) {
        ...
        describe('cordova.InAppBrowser', function() {
            ...
            it('inappbrowser.spec.2 should contain open function', function() {
                ...
                expect(cordova.InAppBrowser.open).toEqual(
        ...
        describe('open method', function() {
            ...
            it('inappbrowser.spec.3 should return InAppBrowser instance with required methods', function() {
                iabInstance = cordova.InAppBrowser.open(
                    url,
                    '_blank',
                    platformOpts,
                );
                ...
                expect(iabInstance.addEventListener).toEqual(
                    jasmine.any(Function),
                );
                expect(iabInstance.removeEventListener).toEqual(
                    jasmine.any(Function),
                );
                ...
                expect(iabInstance.executeScript).toEqual(
                    jasmine.any(Function),
                );
            ...
            it('inappbrowser.spec.4 should support loadstart and loadstop events', function(done) {
                var onLoadStart = jasmine
                    .createSpy('loadstart event callback')
            ...
            it('inappbrowser.spec.7 should support message event', function(done) {
                if (!isAndroid && !isIos) {
                    return pending(
                        cordova.platformId +
                            " platform doesn't support message event",
                    );
                }
        ...
        function logEvent(e) {
            ...
            if (e.type === 'loadstart') {
                if (e.url === lastLoadStartURL) {
                    alert(
                        'Unexpected: loadstart fired multiple times for the same URL.',
            ...
            if (e.type === 'loadstop' || e.type === 'loaderror') {
                if (e.url !== lastLoadStartURL) {
                    alert(
                        'Unexpected: ' +
                            e.type +
                            " event.url != loadstart's event.url",
                    ); // eslint-disable-line no-undef
                }
                if (numExpectedRedirects === 0 && counts.loadstart !== 1) {
                    ...
                    if (!(e.type === 'loaderror' && counts.loadstart === 0)) {
                        alert(
                            'Unexpected: got multiple loadstart events. (' +
                                counts.loadstart +
                                ')',
                        ); // eslint-disable-line no-undef
                    }
                    ...
                    counts.loadstart < numExpectedRedirects + 1
                ) {
                    alert(
            ...
            if (e.type === 'exit') {
                ...
                if (numStopEvents === 0 && !wasReset) {
                    alert(
                        'Unexpected: browser closed without a loadstop or loaderror.',
                    ); // eslint-disable-line no-undef
                } else if (numStopEvents > 1) {
                    alert(
                        'Unexpected: got multiple loadstop/loaderror events.',
                    ); // eslint-disable-line no-undef
                }
            }
        }
    ...
    if (isIos) {
        createActionButton(
            ...
            function() {
                ...
                var button = webviewToggle.getElementsByClassName(
                    'topcoat-button',
                )[0];
    ...
    createActionButton(
        ...
        function() {
            doOpen(
                localhtml,
                'random_string',
                'toolbarposition=top,location=no',
            );
        },
    ...
    createActionButton(
        ...
        function() {
            var ref = cordova.InAppBrowser.open(
                'https://google.com',
                '_blank',
                'location=yes' + (platformOpts ? ',' + platformOpts : ''),
            );
